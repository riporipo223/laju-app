...
- [ ] T4.1 — ~~Circle / Clan~~ Club, renamed 2026-09-23, **display name changed to "Circle" 2026-09-26** (internal `club` unchanged) — scoped 2026-09-23 (§4.24 AC1-AC24, incl. Premium admin tools — T4.8 merged in; open points closed 2026-09-24 except delete-vs-archive, analytics period/N, challenge details), built after T4.15 (lihat [phase-4-backlog.md](./phase-4-backlog.md))
  ...
  - [x] T4.1b — backend: create (free, Premium gate reverted 2026-09-26, `d46734f`) + browse/join/leave/member list + kick-member (§4.24 AC23, `3278e6c`, owner-kick guard fixed `b1f857e`) + member cap enforcement (§4.24 AC22, `ca1d9f2`) all built — `POST`/`GET /api/clubs`, `GET`/`DELETE /api/clubs/[id]/members` (self-leave, kick another member via a `user_id` body gated on caller owner/admin — target `owner` role rejected, AC16), `POST /api/clubs/[id]/join` (409 `circle_full` at 20 members for a Free Circle owner / 100 for Premium, checked live via `isPremiumClub`) — 12/12 create + 12/12 members-route + 12/12 join-route tests pass (all TDD: failing test first). Internal leaderboard is separate (T4.17).
    **Circle Challenge + Analytics, backend only (§4.24 AC11-AC15/AC24), same day:**
    - Schema (`e729a9e`, not yet applied to production): `club_challenge` + `club_membership_history` — the latter exists solely so a departed member's past contribution keeps counting toward a challenge's collective total without threading writes into the run-submission pipeline (see the migration's own comment). Foundation wiring into create/join/leave/kick, `2e6192e`, 36 tests.
    - `POST /api/clubs/[id]/challenge` (create, `daba8ec`, 10 tests): gated owner/admin + Premium Club (`isPremiumClub` reused, `not_premium_club` shape), at most one open challenge per Circle.
    - `GET /api/clubs/[id]/challenge` (progress/ranking, `35b22e1`, 12 pure-function + 5 route tests): visible to any current member, not Premium-gated (AC24 only locks starting new ones). Status computed live, never closes early.
    - `DELETE /api/clubs/[id]/challenge/cancel` (owner only, `1147487`, 6 tests): freezes ranking as final record. **Push notification to participants NOT built — zero remote push infrastructure exists anywhere in this codebase (grepped for APNs/PushKit/device tokens; only local client-scheduled notifications exist), a separate infra task, blocked and documented in the route's own comment.**
    - `GET /api/clubs/[id]/analytics` (`2fd26f6`, 5 pure-function + 5 route tests): gated owner/admin + Premium Club, reuses §4.20's exact "active member" definition, rolling 30 days, top-5 by distance (a judgment call, not a documented sort-key decision — flagged in the code comment).
    - **iOS UI built same-week, separate session** — migration `20260926170000` applied to production first (`b6db974`; also repaired migration history for 7 earlier migrations found genuinely live but untracked, verified table-by-table via read-only query before repairing). Networking layer (`6c5cf31`): DTOs + `APIClient` methods for all 3 units.
      - **Unit 1 — Create Challenge** (`08808be`): distance/duration toggle, future-only deadline. 403 `not_premium_club` shows `PremiumCircleUpsellView` (fresh view — checked first, nothing reusable existed since the old `PremiumUpsellView` was deleted). 409 `challenge_active` shows a clear message. 3 tests, TDD.
      - **Unit 2 — View + Cancel Challenge** (`97179d7`): progress vs target, status, individual ranking, empty state. Cancel is owner-only with a destructive confirmation alert. 4 tests, TDD.
      - **Unit 3 — Circle Analytics** (`88d8248`): total distance/points, active member count, top-5 by distance (confirmed 2026-09-26). Same Premium upsell reused, not duplicated. Entry hidden for non-owner/admin. 2 tests, TDD.
      - Both Challenge and Analytics entries wired into `ClubMemberListView` (the per-Circle detail screen — `ClubHomeView` itself has no single `clubId` in scope).
      - **Verification: BUILD SUCCEEDED, 246/246 iOS tests pass.** Full logged-in simulator walkthrough NOT done — a fresh simulator has no real account and the `LAJU_DEBUG_ACCESS_TOKEN` bypass needs real Supabase tokens (same limitation hit and disclosed during the Circle rename task) — stated plainly rather than claimed.
    **AC24 freeze policy: now fully covered, across two sessions** — refuse-new-members-at-cap (`ca1d9f2`, prior session) + refuse-new-challenge/refuse-analytics-view when Premium lapses (this session's `isPremiumClub` reuse in both routes, checked live, not cached). No single "freeze" feature was built as its own unit — AC24 turned out to already be satisfied by each route's own existing Premium gate, not a gap needing separate work.
  - [x] T4.1c — iOS UI — Create Circle (Premium upsell gate removed 2026-09-26, `PremiumUpsellView.swift` deleted — no other caller, `6923e05`) + `ClubBrowseView` (+ clear "Circle sudah penuh." message on cap-full join, `ca1d9f2`) + `ClubMemberListView` (+ kick-member button, owner/admin only, `3278e6c`) all built, wired into the Circle tab (display copy renamed Club→Circle 2026-09-26, `15dc3b1` — internal type/file names unchanged; not "Club Saya" in You tab — moved per T4.2c's 2026-09-24 nav change) — BUILD SUCCEEDED, 237/237 tests pass. **Still open: freeze policy UI (blocked on the backend work above)**
...
- [x] T4.22 — Real-time anti-cheat warning (Track), added 2026-09-26 — §4.29 AC1-AC5. **Built 2026-09-27, backend + iOS.** Backend (`49698cd`): new `checkSevereSpeedViolation` (5 consecutive T2.8 speed-jump detections, same reused 25 km/h `SPEED_CAP_KMH`, within a rolling 2-minute window) forces `resolveRunStatus` to `rejected` independent of `excluded_pct`; trust-score gets a new `SEVERE_SPEED_VIOLATION_DECAY = 0.2` (starting value, 2x `HIGH_FLAG_DECAY`, needs real calibration — not decided in product-spec.md §4.29, flagged there as its own calibration item). 21 new backend tests (6 detector + 3 status-override + 3 trust-score, TDD). iOS (`fde7662`): `SevereSpeedViolationDetector` replicates the same rule client-side for a neutral, advisory-only warning banner (ADR-0008 — server re-derives the real outcome on sync); documented as a third, distinct check from the existing 12 m/s raw-fix filter and jitter floor so the three don't get confused. 6 new iOS tests, BUILD SUCCEEDED, 256/256 pass. Real-device/simulator visual verification not done (same disclosed limitation as recent tasks — no real account on a fresh simulator).
