...
Depends on: [product-spec.md](./product-spec.md), [tech-spec.md](./tech-spec.md),
[architecture.md](./architecture.md), [database-api-spec.md](./database-api-spec.md)
...
- Minimal region validation, mirroring the Season pattern above: schema
  ...
  pre-submission blocking) and per-region leaderboard granularity are
  Fase 3 — this phase only needs "does this user have a region", not "is
  this region a valid catalog entry" or "does the UI stop them early".
- `LEADERBOARD_SCOPE` schema (part of core migration, alongside
  `LeaderboardEntry`) — migrated here, not Fase 3, because the global
  leaderboard needs an explicit `scope_type='global'` row from the start
  (database-api-spec.md §1). Fase 3 only adds local-scope rows to this
  same table.
...
- Mobile onboarding UX that blocks the run-start flow client-side before
  ...
  experience on top of it, not the guarantee itself.
- `LeaderboardEntry` precompute extended to per-scope aggregation
  (`kecamatan`, `kabupaten_kota`, `provinsi`), including the
  `insufficient_data` handling from architecture.md §4, persisted in the
  `LEADERBOARD_SCOPE` table (database-api-spec.md §1) that Fase 2 already
  migrated (this phase only adds local-scope rows to it).
- Full Season lifecycle (`upcoming → active → ended` transitions) added
  ...
  reset (lifetime points/level untouched).
- Season info screen + countdown.

**Definition of Done:**
- Local leaderboard filters correctly by all three granularity levels and
  handles low-density regions per product-spec AC 4.6.2.
- A season can be closed and a new one opened without corrupting lifetime
  user points/levels; historical final rank is retained per product-spec
  AC 4.7.3.

**Dependency note:** this phase depends on Fase 2's anti-cheat and
precompute infrastructure already being solid — local leaderboards
multiply the number of scopes being computed, so any inefficiency or
correctness gap in the precompute job from Fase 2 gets amplified here.
...
- Route map visualization (Mapbox integration).
