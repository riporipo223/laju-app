...
(Global / Local: Kecamatan → Kabupaten/Kota → Provinsi) → Progress terbentuk →
Run again
...
- Leaderboard lokal (granular sampai kecamatan) punya cukup kepadatan user
  di fase awal untuk terasa relevan, bukan kosong
...
- Local leaderboard hyperlocal expansion di luar kecamatan/kabupaten-kota/
  provinsi (mis. kelurahan/desa) — expand begitu density user cukup.
  Catatan: leaderboard lokal 3-tier (kecamatan/kabupaten-kota/provinsi) itu
  sendiri sudah **v1** (Fase 3), bukan future — yang future cuma ekspansi
  granularitas lebih lanjut di atasnya.
