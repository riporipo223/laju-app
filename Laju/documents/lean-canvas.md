...
Local. Beda dari kompetitor yang gamefikasinya cuma badge kosmetik (Nike Run
Club) atau sosial-driven (Strava kudos) — Laju punya sistem progres
persisten dengan konsekuensi nyata (naik/turun rank per season, positioning
lokal yang relevan secara geografis).
...
- Global leaderboard
- Local leaderboard (granularity: kecamatan → kabupaten/kota → provinsi;
  mengikuti hierarki administratif Indonesia — "daerah" dan campuran
  kabupaten/kota sebagai tier terpisah dihapus, lihat database-api-spec.md)
...
- Maps API (biaya per-request perlu dimonitor ketat — ini item cost yang
  paling sensitif terhadap growth, worth eksplorasi alternatif open-source
  seperti Mapbox/OpenStreetMap di awal untuk tekan biaya)
- Backend
