...
// `.from("club_member").select(...).eq(...).order(...)` — the member list query.
const listChain = {
...
function deleteRequest() {
  return new Request("https://example.com/api/clubs/club-1/members", { method: "DELETE", headers: { authorization: "Bearer valid-jwt" } });
}
